...
export default function SubjectPage() {
  ...
  const { syllabus, loading } = useData();
  
  const subject = syllabus ? syllabus[subjectName] : null;
  ...
  if (loading) {
    return (
      <div className="container" style={{ textAlign: 'center', paddingTop: 'var(--space-3xl)' }}>
        <p style={{ color: 'var(--text-secondary)' }}>Loading syllabus...</p>
      </div>
    );
  }
  ...
  if (!subject) {
    return (
      <div className="container" style={{ textAlign: 'center', paddingTop: 'var(--space-3xl)' }}>
        ...
        <button className="btn btn--primary" onClick={() => router.push('/')} style={{ marginTop: 'var(--space-md)' }}>
          Go Home
        </button>
      </div>
    );
